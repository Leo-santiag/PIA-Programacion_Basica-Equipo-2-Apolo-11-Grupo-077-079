...
def validar_nombre_pokemon_API_Conect(nombre):
...
def obtener_datos_pokemon_API_Conect(nombre):
    if not validar_nombre_pokemon_API_Conect(nombre):
...
#solo se usa si no se tienen tablas.json 
